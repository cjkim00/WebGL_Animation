//
//GEAR LOOKS BEST WHEN SPOKES = TEETH
//LESS THAN 4 SPOKES CREATES GAPS

function cannonGear(numTeeth, numSpokes) {

    var n = numTeeth*2;
    var spokes = numSpokes * 2;
    var rad = 1.0;
    var outRad = rad * 1.2;
    ...
    for (i = 0; i < n; i++) {
        ...
        var v2 = vec4.create((rad / 5) * Math.cos(ang), (rad / 5) * Math.sin(ang), z);
        var v3 = vec4.create((rad / 5) * Math.cos(ang + angInc), (rad / 5) * Math.sin(ang + angInc), z);
    ...
    function drawSpokesFrontBack(vertices, colors, normals, isFront) {
        ...
        for (var i = 0; i < spokes; i++) {
            ...
            if (drawTooth) {
                vertices.push(0, 0, (isFront) ? z : -z,
                    rad2 * Math.cos(ang), rad2 * Math.sin(ang), (isFront) ? z : -z,
                    rad2 * Math.cos(ang + angInc), rad2 * Math.sin(ang + angInc), (isFront) ? z : -z)
                colors.push(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5);
                normals.push(0, 0, (isFront) ? 1 : -1, 0, 0, (isFront) ? 1 : -1, 0, 0, (isFront) ? 1 : -1);
            }
            ang += angInc2;
    ...
    function drawSidesOfSpoke(vertices, colors, normals) {
        ...
        for (i = 0; i < spokes; i++) {
            drawTooth = !drawTooth;
            if (drawTooth) {
                var norm = [innerCircle * Math.cos(ang + angInc / 2), innerCircle * Math.sin(ang + angInc / 2), 0];
                vertices.push(
                ...
                vertices.push(
                    ...
                    innerCircle * Math.cos(ang + angInc), innerCircle * Math.sin(ang + angInc), -z,
                    innerCircle * Math.cos(ang + angInc), innerCircle * Math.sin(ang + angInc), z)
                colors.push(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5)
                normals.push(-norm[0], -norm[1], -norm[2], -norm[0], -norm[1], -norm[2], -norm[0], -norm[1], -norm[2])
                vertices.push(
