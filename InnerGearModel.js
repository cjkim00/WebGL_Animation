...
function createInnerGear(numTeeth, numSpokes) {
    const vertices = [];
    const colors = [];
    const normals = [];
    ...
    var outRad = 0.78; //controls the length of the teeth
    var angInc = 2 * 3.14159 / n;//controls the % of the circle
    var ang = 0;
    var z = 0.1;

    var i;
    var temp = rad * 0.76;//circle outer edge
    var innerEdge = 0.66;//circle inner edge
    ...
    for (i = 0; i < n; i++) {   // tooth walls
        ...
        if (drawTooth) {
            ...
            var norm  = calcNormal(temp * Math.cos(ang), temp * Math.sin(ang), -z,
            ...
            var norm = calcNormal(temp * Math.cos(ang + angInc), temp * Math.sin(ang + angInc), -z,
